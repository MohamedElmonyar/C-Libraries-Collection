...
public:
	...
	string HourMinuteSecound() 
	{
		return to_string(_Hour) + ":" + to_string(_Minute) + ":" + to_string(_Secound);
	};

	static string GetAllDateAsString() 
	{
		time_t t = time(0);
		tm* now = localtime(&t);
		short _Day = now->tm_mday;
		short _Month = now->tm_mon + 1;
		short _Year = now->tm_year + 1900;
		short _Hour = now->tm_hour;
		short _Minute = now->tm_min;
		short _Secound = now->tm_sec;
	    string SecondDate = to_string(_Hour) + ":" + to_string(_Minute) + ":" + to_string(_Secound);
		string OneDate = to_string(_Day) + "/" + to_string(_Month) + "/" + to_string(_Year);
		return OneDate + " - " + SecondDate;
	};

    string GetAllDateasString() 
	{
	    return DateToString() + " - " + HourMinuteSecound();
	};
